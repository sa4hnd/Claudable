...
const ToolResultItem: React.FC<ToolResultItemProps> = ({ action, filePath, content }) => {
  ...
  const getIcon = () => {
    switch (action) {
      ...
      case 'Created':
        return (
          <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 -960 960 960" className="shrink-0 h-3 w-3 text-white" fill="currentColor">
  ...
  const getRelativePath = (path: string) => {
    // Remove /vibe0/my-app-*/ prefix to show only relative path
    const match = path.match(/\/vibe0\/my-app-[^\/]+\/(.+)/);
    if (match) {
      return match[1];
    }
      ...
      >
        ...
        {content && (
          <svg 
            ...
            className={`ml-auto text-gray-400 transition-transform ${isExpanded ? 'rotate-90' : ''}`}
          >
