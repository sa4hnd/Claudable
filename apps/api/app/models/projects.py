...
class Project(Base):
    ...
    # Sandbox Integration
    sandbox_id: Mapped[Optional[str]] = mapped_column(String(128), nullable=True, index=True)  # VibeKit sandbox ID
    sandbox_status: Mapped[Optional[str]] = mapped_column(String(32), default="inactive")  # active, inactive, error
    host_url: Mapped[Optional[str]] = mapped_column(String(512), nullable=True)  # Sandbox host URL
    last_sandbox_activity: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    ...
    sandbox_sessions = relationship("SandboxSession", back_populates="project", cascade="all, delete-orphan")
